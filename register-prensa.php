...
<body>
          ...
          <div class="card bg-dark text-white" style="border-radius: 1rem;">
            <div class="card-body p-5 text-center">
              <div class="mb-md-5 mt-md-4 pb-5">
                <h2 class="fw-bold mb-2 text-uppercase">Registro: Prensa</h2>
                <p class="text-white-50 mb-5">Introduzca los datos para registrarse correctamente!</p>
                <form action="" method="post">
                  <div class="form-outline form-white mb-4">
                    <input type="text" name="nombre" class="form-control form-control-lg" />
                    <label class="form-label" for="usuario">Nombre de Empresa</label>
                  </div>
                  <div class="form-outline form-white mb-4">
  ...
  if (isset($_POST["registrar"]) && isset($_POST["nombre"]) && isset($_POST["correo"]) && isset($_POST["contrasenia2"]) && isset($_POST["rep-contrasenia2"])) {
    $nombre = $_POST["nombre"];
    ...
    if ($contraseña1 == $contraseña2 && !empty($nombre) && !empty($correo) && !empty($contraseña1)) {
      ...
      if ($mysqli->connect_error) {
        ...
        $sql = "INSERT INTO Prensa (nombre, correo, contrasenia) VALUES (?, ?, ?)";
        $stmt = $mysqli->prepare($sql);
        if ($stmt) {
          // Enlazamos los parámetros con los valores proporcionados por el usuario
          $stmt->bind_param("sss", $nombre, $correo, $contraseña1);
          // Ejecutamos la consulta
          if ($stmt->execute()) {
            header("location: login_prensa.php");
            exit();
          } else {
